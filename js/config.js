...
class ConfigManager {
    ...
    init() {
        ...
        window.addEventListener('message', (event) => {
            // Handle code verifier request from callback
            if (event.data && event.data.type === 'request_code_verifier') {
                console.log('Callback is requesting code verifier');
                const savedState = localStorage.getItem('spotify_auth_state');
                
                if (event.data.state === savedState) {
                    const codeVerifier = localStorage.getItem('spotify_code_verifier');
                    console.log('Sending code verifier to callback');
                    
                    event.source.postMessage({
                        type: 'code_verifier_response',
                        codeVerifier: codeVerifier
                    }, '*');
                } else {
                    console.error('State mismatch in code verifier request');
                }
                return;
            }
            
            // Handle token data from callback
            if (event.data && event.data.type === 'spotify_token') {
                console.log('Received Spotify token from callback:', event.data);
                ...
                if (tokenData.access_token && tokenData.state) {
                    ...
                    if (tokenData.state === savedState) {
                        ...
                        console.error('State mismatch! Expected:', savedState, 'Got:', tokenData.state);
                        this.showNotification('Spotify authentication failed (state mismatch)', 'error');
                    }
                } else {
    ...
    connectSpotify() {
        ...
        // Store state and other needed data in the state parameter itself (base64 encoded)
        const stateData = {
            state: state,
            clientId: clientId,
